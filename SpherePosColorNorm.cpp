...
{
	//Effect
	m_pEffect = ContentManager::GetInstance()->Load<ID3DX11Effect>(L"Effects/PosColNorm3D.fx");
	...
	float phiStep = 3.1415f / m_Stacks;

	// do not count the poles as rings
	int numRings = m_Stacks - 1;
	...
	for (int i = 1; i <= numRings; ++i)
	{
		...
		// vertices of ring
		float thetaStep = 2.0f * (float)3.1415f / m_Slices;
	...
	int numRingVertices = m_Slices + 1;

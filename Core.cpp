...
void Core::AddScenes()
{
	...
	Logger::GetInstance()->LogInfo(L"Scenes Initialized!");
}
...
HRESULT Core::RunGame(HINSTANCE hInstance)
{
	...
	gameSettings.isVSyncOn = true; // bij true, cubes bounce
	gameSettings.clearColor = XMFLOAT3(DirectX::Colors::CornflowerBlue);
	...
	std::wcout << L"InitalizeAdapterAndOutput ok." << std::endl;

	//1. Create the Window Class
	hr = CreateWindowClass();
	if(FAILED(hr)) return hr; 
	std::wcout << L"Window Class Created" << std::endl;

	//2. Open the window 
	hr = OpenWindow();
	if(FAILED(hr)) return hr; 
	std::wcout << L"Window Opened" << std::endl;

	//3. Direct3D Init
	hr = InitializeDirect3D();
	if(FAILED(hr)) return hr; 
	std::wcout << L"Direct3D Initialized" << std::endl;

	//4. Initialize Managers
	ContentManager::GetInstance()->Initialize(m_D3D11.pDevice);
	PhysxManager::GetInstance();
	SoundManager::GetInstance();
	Logger::GetInstance()->LogInfo(L"Managers Initialized!");
	...
	while(msg.message != WM_QUIT)
	{
		...
		XMVECTOR clearColor = XMLoadFloat3(&gameSettings.clearColor);
		...
		m_pSwapChain->Present(gameSettings.isVSyncOn ? 1 : 0, 0);
	}
...
HRESULT Core::InitalizeAdapterAndOutput()
{
	...
	while (m_pDXGIFactory->EnumAdapters(i, &m_pDXGIAdapter) != DXGI_ERROR_NOT_FOUND)
	{
		...
		std::wcout << L"Adapter " << i << L": " << adapterDescriptor.Description << std::endl;

		++i;
	}
	std::wcout << i << L" Adapter(s) found" << std::endl;
...
HRESULT Core::OpenWindow()
{
	...
	if (FAILED(hr))return hr;
	std::wcout << "Desktop resolution: " << outputDesc.DesktopCoordinates.right << " x " << outputDesc.DesktopCoordinates.bottom << std::endl;
	//Calculate position (centered)
	RECT R = {0,0,gameSettings.width,gameSettings.height};
...
HRESULT Core::InitializeDirect3D()
{
	...
	{
		Logger::GetInstance()->LogHResult(-1, L"Feature level 10.0+ not supported on this device!");
		exit(-1);
	}
	...
	swapChainDesc.BufferDesc.Width = gameSettings.width;
	swapChainDesc.BufferDesc.Height = gameSettings.height;
	...
	depthStencilDesc.Width     = gameSettings.width;
	depthStencilDesc.Height    = gameSettings.height;
	...
	m_Viewport.Width	= (FLOAT)gameSettings.width;
	m_Viewport.Height	= (FLOAT)gameSettings.height;
...
#pragma region
bool Core::AlertOnFail(HRESULT hr, std::wstring originInfo)
{
...
LRESULT CALLBACK Core::WindowsProcedureStatic(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
		Core* pThisGame = (Core*)GetWindowLongW(hWnd, GWLP_USERDATA);

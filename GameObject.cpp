...
void GameObject::AttachRigidActor(PxRigidActor* pRigidActor)
{
	...
	if(m_pRigidActor != nullptr)
	{
		...
		Logger::GetInstance()->LogWarning(L"GameObject::AttachRigidActor(...) > Overwritting current Actor!");
	}
...
void GameObject::CalculateWorldMatrix(const DirectX::XMMATRIX &parentWorld, bool updateChildren)
{
	...
	DirectX::XMMATRIX matTrans = XMMatrixTranslation(m_Position.x, m_Position.y, m_Position.z);
	DirectX::XMMATRIX matRot = XMMatrixRotationQuaternion(XMLoadFloat4(&m_Rotation));
	DirectX::XMMATRIX matScale = XMMatrixScaling(m_Scale.x, m_Scale.y, m_Scale.z);

	DirectX::XMMATRIX matWorld = XMMatrixMultiply(matScale, matRot);

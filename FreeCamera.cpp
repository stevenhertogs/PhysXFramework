...
FreeCamera::FreeCamera()
	...
	, m_LeftKey{'A'}
{
	BuildProjection(XM_PIDIV4, float(Core::gameSettings.width) / Core::gameSettings.height , 0.01f, 1000.0f);
}
...
void FreeCamera::Update(const SceneContext& sceneContext)
{
	...
	if (input->IsMouseButtonDown(VK_LBUTTON))
	{
		...
		//Logger::GetInstance()->LogInfo(std::to_wstring(xDiff) + std::to_wstring(yDiff));
	...
	if (input->IsKeyboardKeyDown(m_UpKey))
		position += forward * m_Speed * deltaTime;
	else if (input->IsKeyboardKeyDown('S'))
		position -= forward * m_Speed * deltaTime;

	if (input->IsKeyboardKeyDown(m_LeftKey))
		position -= right * m_Speed * deltaTime;
	else if (input->IsKeyboardKeyDown('D'))
		position += right * m_Speed * deltaTime;
